...
export default class ObjAiBase extends AttackableUnit {
    ...
    constructor(
        ...
        if (!string.IsNullOrEmpty(model)) {
            AaSpellData = _game.Config.ContentManager.GetSpellData(
                model + "BasicAttack"
            );
            let baseAttackCooldown =
                1.6 * (1.0 + CharData.AttackDelayOffsetPercent);
            AutoAttackCastTime =
                baseAttackCooldown *
                (0.3 + CharData.AttackDelayCastOffsetPercent);
    ...
    AutoAttackHit(target) {
        ...
        let damage = Stats.AttackDamage.Total;
        if (_isNextAutoCrit) {
            damage *= Stats.CriticalDamage.Total;
        }

        let onAutoAttack = _game.ScriptEngine.GetStaticMethod(
            Model,
            "Passive",
            "OnAutoAttack"
        );
    ...
    ChangeAutoAttackSpellData(newAutoAttackSpellData) {
        AaSpellData = newAutoAttackSpellData;
    }

    /// Sets this AI's current auto attack to the given auto attack. *NOTE*: Will be deprecated when spells are fully implemented.
    /// <param name="newAutoAttackSpellDataName">Name of the auto attack to use.</param>
    ChangeAutoAttackSpellData(newAutoAttackSpellDataName) {
        AaSpellData = _game.Config.ContentManager.GetSpellData(
            newAutoAttackSpellDataName
        );
    }
    ...
    ClassifyTarget(target) {
        if (
            target instanceof ObjAiBase &&
            target.TargetUnit != null &&
            target.TargetUnit.IsInDistress()
        ) {
            // If an ally is in distress, target this unit. (Priority 1~5)
            let ai = target;
            // Champion attacking an allied champion
            if (ai instanceof Champion && ai.TargetUnit instanceof Champion)
                return ClassifyUnit.CHAMPION_ATTACKING_CHAMPION;
            // Champion attacking lane minion
            else if (
                ai instanceof Champion &&
                ai.TargetUnit instanceof LaneMinion
            )
                return ClassifyUnit.CHAMPION_ATTACKING_MINION;
            // Champion attacking minion
            else if (ai instanceof Champion && ai.TargetUnit instanceof Minion)
                return ClassifyUnit.CHAMPION_ATTACKING_MINION;
            // Minion attacking an allied champion.
            else if (ai instanceof Minion && ai.TargetUnit instanceof Champion)
                return ClassifyUnit.MINION_ATTACKING_CHAMPION;
            // Minion attacking lane minion
            else if (
                ai instanceof Minion &&
                ai.TargetUnit instanceof LaneMinion
            )
                return ClassifyUnit.MINION_ATTACKING_MINION;
            // Minion attacking minion
            else if (ai instanceof Minion && ai.TargetUnit instanceof Minion)
                return ClassifyUnit.MINION_ATTACKING_MINION;
            // Turret attacking lane minion
            else if (
                ai instanceof BaseTurret &&
                ai.TargetUnit instanceof LaneMinion
            )
            ...
            // Turret attacking minion
                ...
                ai instanceof BaseTurret &&
                ai.TargetUnit instanceof Minion
            )
                return ClassifyUnit.TURRET_ATTACKING_MINION;
        }
        ...
        } else if (target instanceof BaseTurret) return ClassifyUnit.TURRET;
        else if (target instanceof Champion) return ClassifyUnit.CHAMPION;
        else if (target instanceof Inhibitor && !target.IsDead)
            return ClassifyUnit.INHIBITOR;
        else if (target instanceof Nexus) return ClassifyUnit.NEXUS;
    ...
    OnCollision(collider, isTerrain = false) {
        if (!isTerrain) {
            ...
            if (
                !(
                    collider instanceof Champion ||
                    collider instanceof BaseTurret
                )
            ) {
                return;
            }
        ...
        if (!IsPathEnded()) {
            ...
            let safePath = _game.Map.NavigationGrid.GetPath(
                Position,
                _game.Map.NavigationGrid.GetClosestTerrainExit(Waypoints.Last())
            );
            if (safePath != null) {
    ...
    DashToTarget(
        ...
        IsDashing = true;
        DashTime =
            Vector2.Distance(target.Position, Position) / (dashSpeed * 0.001);
        DashElapsedTime = 0;
        DashSpeed = dashSpeed;

        SetWaypoints([Position, target.Position], false);
    ...
    RecalculateAttackPosition() {
        // If we are already where we should be, which means we are in attack range and not colliding, then keep our current position.
        if (
            TargetUnit != null &&
            !TargetUnit.IsDead &&
            Vector2.DistanceSquared(Position, TargetUnit.Position) >
                CollisionRadius * CollisionRadius &&
            Vector2.DistanceSquared(Position, TargetUnit.Position) <=
                Stats.Range.Total * Stats.Range.Total
        ) {
            return false;
        }
        let objects = _game.ObjectManager.GetObjects();
        let usedPositions = [];
        let isCurrentlyOverlapping = false;

        let thisCollisionCircle = new CirclePoly(
            TargetUnit?.Position ?? Position,
            CollisionRadius + 10
        );

        for (let gameObject in objects) {
            let unit = gameObject.Value; // as IAttackableUnit;
            if (
                unit == null ||
                unit.NetId == NetId ||
                unit.IsDead ||
            ...
            let targetCollisionCircle = new CirclePoly(
                unit.Position,
        ...
        if (isCurrentlyOverlapping) {
            ...
            let targetCircle = new CirclePoly(
                TargetUnit.Position,
                Stats.Range.Total,
                72
            );
            //Find optimal position...
            for (let point in targetCircle.Points.OrderBy((x) =>
                Vector2.DistanceSquared(Position, x)
            )) {
                if (!_game.Map.NavigationGrid.IsVisible(point)) {
                    continue;
                }
                let positionUsed = false;
                for (let circlePoly in usedPositions) {
                    if (
                        circlePoly.CheckForOverLaps(
                            new CirclePoly(point, CollisionRadius + 10, 20)
                        )
                    ) {
                        positionUsed = true;
                    }
    ...
    RefreshWaypoints() {
        ...
        if (
            !IsDashing &&
            Vector2.DistanceSquared(Position, TargetUnit.Position) <=
                (Stats.Range.Total - 2) * (Stats.Range.Total - 2)
        ) {
            StopMovement();
        }
        ...
        // Otherwise, move to the target.
            ...
            let targetPos = TargetUnit.Position;
            if (
                !_game.Map.NavigationGrid.IsWalkable(
                    targetPos,
                    TargetUnit.CollisionRadius
                )
            ) {
                targetPos = _game.Map.NavigationGrid.GetClosestTerrainExit(
                    targetPos,
                    CollisionRadius
                );
            }

            let newWaypoints = _game.Map.NavigationGrid.GetPath(
                Position,
                targetPos
            );
    ...
    UpdateAutoAttackTarget(diff) {
        if (
            HasCrowdControl(CrowdControlType.DISARM) ||
            HasCrowdControl(CrowdControlType.STUN)
        ) {
            return;
        }
        ...
        if (TargetUnit != null) {
            if (
                TargetUnit.IsDead ||
                (!_game.ObjectManager.TeamHasVisionOn(Team, TargetUnit) &&
                    !(TargetUnit instanceof IBaseTurret) &&
                    !(TargetUnit instanceof IObjBuilding))
            ) {
                SetTargetUnit(null);
                IsAttacking = false;
                _game.PacketNotifier.NotifySetTarget(this, null);
                ...
                _autoAttackCurrentDelay += diff / 1000.0;
                if (
                    _autoAttackCurrentDelay >=
                    AutoAttackCastTime / Stats.AttackSpeedMultiplier.Total
                ) {
                    if (!IsMelee) {
                ...
                RefreshWaypoints();
                _isNextAutoCrit =
                    _random.Next(0, 100) < Stats.CriticalChance.Total * 100;
                if (_autoAttackCurrentCooldown <= 0) {
                    ...
                    let attackType = IsMelee
                        ? AttackType.ATTACK_TYPE_MELEE
                        : AttackType.ATTACK_TYPE_TARGETED;
                    _game.PacketNotifier.NotifyOnAttack(
                        this,
                        TargetUnit,
                        attackType
                    );
        ...
        if (_autoAttackCurrentCooldown > 0) {
            _autoAttackCurrentCooldown -= diff / 1000.0;
        }
    }
