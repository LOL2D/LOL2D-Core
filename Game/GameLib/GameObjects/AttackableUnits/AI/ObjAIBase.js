import AttackableUnit from "../AttackableUnit.js";
        ...
        export default class ObjAiBase extends AttackableUnit
    {
        ...
        /// Unit this AI will auto attack when it is in auto attack range.
         ...
         constructor( game,  model,  stats,  collisionRadius = 40,
             position = new Vector2(),  visionRadius = 0,  netId = 0,  team = TeamId.TEAM_NEUTRAL)
        {
            super(game, model, stats, collisionRadius, position, visionRadius, netId, team)
            ...
            if (!string.IsNullOrEmpty(model))
            {
                ...
                let baseAttackCooldown = 1.6 * (1.0 + CharData.AttackDelayOffsetPercent);
                AutoAttackCastTime = baseAttackCooldown * (0.3 + CharData.AttackDelayCastOffsetPercent);
                 ...
                 AutoAttackHit( target)
        {
            ...
            let onAutoAttack = _game.ScriptEngine.GetStaticMethod(Model, "Passive", "OnAutoAttack");
        ...
        {
            if (target instanceof ObjAiBase && target.TargetUnit != null && target.TargetUnit.IsInDistress()) // If an ally is in distress, target this unit. (Priority 1~5)
            {
                let ai = target;
                ...
                else if(ai instanceof Champion && ai.TargetUnit instanceof LaneMinion)
                ...
                else if(ai instanceof Minion && ai.TargetUnit instanceof LaneMinion)
                ...
                else if(ai instanceof BaseTurret && ai.TargetUnit instanceof LaneMinion)
                    return ClassifyUnit.TURRET_ATTACKING_MINION;
                // Turret attacking minion
                else if(ai instanceof BaseTurret && ai.TargetUnit instanceof Minion)
                    return ClassifyUnit.TURRET_ATTACKING_MINION;
                ...
                else if (target instanceof BaseTurret)
                ...
                else if(target instanceof Nexus)
        ...
        OnCollision( collider, isTerrain = false)
        {
            if (!isTerrain)
            {
                ...
                if (!(collider instanceof Champion || collider instanceof BaseTurret))
                {
                    return;
                }
            }
            ...
            if (!IsPathEnded())
            {
                ...
                let safePath = _game.Map.NavigationGrid.GetPath(Position, _game.Map.NavigationGrid.GetClosestTerrainExit(Waypoints.Last()));
                if (safePath != null)
                {
        ...
        )
        {
            ...
            IsDashing = true;
            DashTime = Vector2.Distance(target.Position, Position) / (dashSpeed * 0.001);
            DashElapsedTime = 0;
            DashSpeed = dashSpeed;
         ...
         RecalculateAttackPosition()
        {
            ...
            let thisCollisionCircle = new CirclePoly(TargetUnit?.Position ?? Position, CollisionRadius + 10);

            foreach (let gameObject in objects)
            {
                ...
                let targetCollisionCircle = new CirclePoly(unit.Position, unit.CollisionRadius + 10);
            ...
            if (isCurrentlyOverlapping)
            {
                ...
                let targetCircle = new CirclePoly(TargetUnit.Position, Stats.Range.Total, 72);
                //Find optimal position...
                foreach (let point in targetCircle.Points.OrderBy(x => Vector2.DistanceSquared(Position, x)))
                {
                    ...
                    foreach (let circlePoly in usedPositions)
                    {
        ...
        RefreshWaypoints()
        {
            ...
            if (!IsDashing && Vector2.DistanceSquared(Position, TargetUnit.Position) <= (Stats.Range.Total - 2) * (Stats.Range.Total - 2))
            {
                StopMovement();
            }
            ...
            else
            {
                ...
                let newWaypoints = _game.Map.NavigationGrid.GetPath(Position, targetPos);
         ...
         void SetTargetUnit(IAttackableUnit target)
        {
         ...
         override void Update(float diff)
        {
            base.Update(diff);
        ...
        /// Updates this AI's current target and auto attack actions depending on conditions such as crowd control, death state, vision, distance to target, etc.
         ...
         void UpdateAutoAttackTarget(float diff)
        {
            ...
            if (TargetUnit != null)
            {
                ...
                else if (Vector2.DistanceSquared(Position, TargetUnit.Position) <= Stats.Range.Total * Stats.Range.Total && !IsDashing)
                {
                    ...
                    if (_autoAttackCurrentCooldown <= 0)
                    {
                        ...
                        let attackType = IsMelee ? AttackType.ATTACK_TYPE_MELEE : AttackType.ATTACK_TYPE_TARGETED;
        ...
        /// Sets this unit's move order to the given order.
         ...
         virtual void UpdateMoveOrder(MoveOrder order)
        {
            MoveOrder = order;
        }
